...
def main_llamaparser():
    ...
    with open(os.path.join(LLAMAPARSER_OUTPUT_DICT_PATH, 'full_parsed_results.jsonl'), "w") as out_file:
        for file_path in new_file_path_list: #[file_path_list[-1]]:#[:-2]:
...
#         )
    ...
    # 업로드 파일을 캐시 폴더에 저장
    @st.cache_resource(show_spinner="Loading and indexing the uploaded docs – hang tight! This should take 1-2 minutes.")
